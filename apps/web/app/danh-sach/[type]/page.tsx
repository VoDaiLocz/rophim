  ...
  const fetchMoviesFromAPI = useCallback(
    (page: number) => {
      if (type === "phim-moi") {
        ...
        return getMoviesByType(type, page);
      }
    },
    [type],
  );
    ...
    const fetchInitial = async () => {
      ...
      try {
        ...
        const responses = await Promise.all(
          pages.map((p) => fetchMoviesFromAPI(p)),
        );
        ...
        if (validResults.length > 0) {
          ...
          const unique = Array.from(
            new Map(allItems.map((m) => [m._id, m])).values(),
          );
      ...
      <div className="pt-24 container mx-auto px-4 lg:px-8 pb-20">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-32 space-y-6">
            ...
            <p className="text-white/40 animate-pulse text-xs font-black uppercase tracking-[0.3em]">
              Cố vấn dữ liệu đang nạp...
            </p>
          </div>
        ) : (
          <InfiniteMovieGrid
